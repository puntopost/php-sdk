...
class ParcelTest extends TestCase
{
    ...
    private function buildMinimalParcelData(): array
    {
        return [
            ...
            'destination' => ['id' => 'DEST', 'external_id' => 'ED1', 'type' => 'pudo', 'name' => 'PUDO Dest',
                'description' => '', 'address' => ['postal_code' => '06600', 'city' => 'CDMX', 'address' => 'Calle 2'],
    ...
    public function testFromArrayWithFullPayload(): void
    {
        ...
        $data['origin'] = ['id' => 'ORIG', 'external_id' => 'EO1', 'type' => 'pudo', 'name' => 'PUDO Orig',
            'description' => '', 'address' => ['postal_code' => '64000', 'city' => 'MTY', 'address' => 'Calle 1'],
            'schedule' => 'Lun-Vie', 'enabled' => true, 'created_at' => '2024-01-01T00:00:00+00:00'];
    ...
    public function testFromArrayIgnoresNonArrayStatusHistoryEntries(): void
    {
        $data = $this->buildMinimalParcelData();
        $data['status_history'] = [
            ['status' => 'created', 'when' => '2024-01-15T10:00:00+00:00'],
        ...
        $this->assertCount(1, $parcel->getStatusHistory());
    }

    public function testFromArrayWithEmptyArrayProducesDefaults(): void
    {
        $parcel = Parcel::fromArray([]);

        $this->assertSame('', $parcel->getId());
        $this->assertSame('', $parcel->getTracking());
        $this->assertSame('', $parcel->getQrTracking());
        $this->assertNull($parcel->getLabel());
        $this->assertNull($parcel->getQrLabel());
        $this->assertSame('', $parcel->getStatus()->getValue());
        $this->assertCount(0, $parcel->getStatusHistory());
        $this->assertNull($parcel->getOrigin());
        $this->assertNull($parcel->getExpireAt());
        $this->assertInstanceOf(DateTimeImmutable::class, $parcel->getCreatedAt());
    }
    ...
    {
        $parcel = Parcel::fromArray(['alpha' => 1, 'beta' => 'two', 'gamma' => [3], 'http_method' => 'DELETE']);
