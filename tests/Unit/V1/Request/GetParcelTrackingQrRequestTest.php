...
class GetParcelTrackingQrRequestTest extends TestCase
{
    public function testConstructorStoresIdentifier(): void
    {
        $request = new GetParcelTrackingQrRequest('MXT0000000001');

        $this->assertSame('MXT0000000001', $request->getIdentifier());
    }

    public function testFromParcelUsesTrackingNumber(): void
    {
        $parcel = $this->buildParcel();

        $request = GetParcelTrackingQrRequest::fromParcel($parcel);

        $this->assertSame('MXT0000000001', $request->getIdentifier());
    }

    public function testFromParcelResponseUsesTrackingNumber(): void
    {
        $response = new ParcelDetailResponse($this->buildParcel());

        $request = GetParcelTrackingQrRequest::fromParcelResponse($response);

        $this->assertSame('MXT0000000001', $request->getIdentifier());
    }

    private function buildParcel(): Parcel
    {
